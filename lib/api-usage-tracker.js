...
// Client créé à la demande (et non au chargement du module) : une absence
// de SUPABASE_SERVICE_ROLE_KEY au démarrage ne doit pas faire planter
// l'ensemble de la fonction serverless qui importe ce module (ex: analyse-tirage.js).
let supabase = null;
...
async function logApiUsage({
    ...
    errorMessage = null,
    requestDurationMs,
    costUsd: costUsdOverride // coût déjà connu (ex: image OpenAI facturée à l'appel, pas au
                              // token) — évite de forcer tous les appelants à passer par la
                              // grille tarifaire Anthropic ci-dessus, qui ne s'applique qu'aux
                              // modèles Claude.
}) {
    try {
        const totalTokens = (requestTokens || 0) + (responseTokens || 0);

        // Calculer le coût : override direct si fourni, sinon grille Anthropic par tokens.
        let costUsd;
        if (typeof costUsdOverride === 'number') {
            costUsd = costUsdOverride;
        } else {
            const pricing = ANTHROPIC_PRICING[modelName] || ANTHROPIC_PRICING['claude-haiku-4-5'];
            costUsd = (requestTokens || 0) * pricing.input_per_mtok / 1000000 +
...
async function getUsageStats(startDate, endDate = null) {
    try {
        ...
        // Stats par API (ex: distinguer Claude / OpenAI images / OpenAI audio — plusieurs
        // fournisseurs partagent la même table, api_name est le seul champ qui les distingue
        // fiablement, model_name pouvant se recouper).
        data.forEach(log => {
            if (!stats.byApi[log.api_name]) {
                stats.byApi[log.api_name] = { calls: 0, successCalls: 0, costEur: 0 };
            }
            stats.byApi[log.api_name].calls++;
            if (log.status === 'success') stats.byApi[log.api_name].successCalls++;
            stats.byApi[log.api_name].costEur += log.cost_eur || 0;
        });

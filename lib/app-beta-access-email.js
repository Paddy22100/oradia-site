...
// Notifie contact@oradia.fr qu'une nouvelle demande d'accès bêta est arrivée
// (avant l'ajout manuel au groupe testeurs) — pour ne pas avoir à surveiller
// le dashboard en continu pour repérer les nouvelles inscriptions.
async function sendBetaSignupAdminNotification({ email, name }) {
  const apiKey = process.env.BREVO_API_KEY;
  const senderEmail = process.env.BREVO_SENDER_EMAIL;
  const senderName = process.env.BREVO_SENDER_NAME || 'ORADIA';
  if (!apiKey || !senderEmail) {
    console.warn('[AppBetaAccess] Config Brevo manquante (notif admin)');
    return false;
  }

  const who = name ? `${escapeHtml(name)} (${escapeHtml(email)})` : escapeHtml(email);
...
<body style="margin:0;padding:0;background-color:#0a192f;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#0a192f" style="background-color:#0a192f;">
    <tr>
      <td align="center" style="padding:36px 24px;">
        <table role="presentation" width="520" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:520px;" bgcolor="#0a192f">
          <tr>
            <td style="padding:0 0 20px;">
              <p style="margin:0;color:#d4af37;font-family:Georgia,serif;font-size:11px;letter-spacing:0.2em;text-transform:uppercase;">Nouvelle demande — bêta app mobile</p>
            </td>
          </tr>
          <tr>
            <td style="padding:0 0 20px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#162230" style="background-color:#162230;border:1px solid #3a3020;border-radius:12px;">
                <tr>
                  <td style="padding:20px 24px;">
                    <p style="margin:0;color:#e9e7df;font-family:Georgia,serif;font-size:15px;line-height:1.7;"><strong style="color:#d4af37;">${who}</strong> souhaite rejoindre la bêta de l'app Oradia.</p>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td>
              <p style="margin:0;color:#8a7a52;font-family:Georgia,serif;font-size:13px;line-height:1.7;">
                Ajoutez cette personne au groupe testeurs dans Play Console, puis envoyez-lui le lien depuis le dashboard (Mails → Demandes d'accès bêta).
              </p>
            </td>
          </tr>
        </table>
  ...
  const textContent = `${who} souhaite rejoindre la bêta de l'app Oradia. Ajoutez-la au groupe testeurs Play Console, puis envoyez le lien depuis le dashboard (Mails > Demandes d'accès bêta).`;
  ...
  try {
    const response = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: `[Oradia] Nouvelle demande d'accès bêta — ${email}`,
        htmlContent,
        textContent
      })
    });
    if (!response.ok) {
      console.error('[AppBetaAccess] Brevo notif admin error:', response.status, await response.text().catch(() => ''));
      return false;
    }
    return true;
  } catch (err) {
    console.error('[AppBetaAccess] notif admin exception:', err.message);
    return false;
  }
}

module.exports = { sendAppBetaAccessEmail, sendBetaSignupAdminNotification, PLAY_STORE_URL };

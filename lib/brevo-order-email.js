...
// Échappement HTML des données saisies par le client (nom, adresse…) : elles étaient
// insérées telles quelles dans le HTML de l'email, ce qui permettait d'y injecter des
// balises (lien, image, mise en forme) depuis le formulaire de commande.
...
// Libellés des offres : l'email affichait l'identifiant technique ("guidance-incluse").
const OFFER_LABELS = {
  'standard': 'Oracle Oradia — Standard',
  'guidance-incluse': 'Oracle Oradia — Guidance Offerte',
  'edition-signature': 'Oracle Oradia — Édition Signature',
  'contribution-libre': 'Contribution libre'
};
function describeOffer(offer) {
  if (!offer) return 'Oracle Oradia';
  return OFFER_LABELS[offer] || offer;
}

const SHIPPING_LABELS = {
  relay: 'Livraison en point relais',
...
// Récapitulatif ligne par ligne (offres + livraison). items = preorders.items
// ([{ offer, quantity }]) ; shipping = { method, priceCents, relayName, relayAddress,
// relayPostalCode, relayCity }. Retourne { html, text }.
function buildOrderRecap({ offer, items, shipping }) {
  const lines = (Array.isArray(items) && items.length ? items : [{ offer, quantity: 1 }])
    .filter(it => it && it.offer)
    .map(it => ({ label: describeOffer(it.offer), quantity: Math.max(1, parseInt(it.quantity, 10) || 1) }));

  const rowHtml = (left, right, strong = false) => `
                  <tr>
                    <td style="padding:6px 0;color:${strong ? '#f0c75e' : '#e8e9eb'};font-family:Georgia,serif;font-size:${strong ? '16px' : '14px'};line-height:1.5;${strong ? 'font-weight:bold;' : ''}">${left}</td>
                    <td align="right" style="padding:6px 0 6px 12px;color:#e8e9eb;font-family:Georgia,serif;font-size:14px;white-space:nowrap;">${right}</td>
                  </tr>`;

  let html = lines.map(l => rowHtml(escapeHtml(l.label), `× ${l.quantity}`, true)).join('');
  const text = lines.map(l => `- ${l.label} × ${l.quantity}`);

  if (shipping && shipping.method) {
    const label = SHIPPING_LABELS[shipping.method] || 'Livraison';
    const price = shipping.method === 'hand_delivery' || !Number(shipping.priceCents)
      ? 'Offerte'
      : formatEuros(shipping.priceCents);
    html += rowHtml(escapeHtml(label), price);
    text.push(`- ${label} : ${price}`);
    if (shipping.method === 'relay' && shipping.relayName) {
      const addr = [shipping.relayAddress, [shipping.relayPostalCode, shipping.relayCity].filter(Boolean).join(' ')]
        .filter(Boolean).join(', ');
      html += `
                  <tr>
                    <td colspan="2" style="padding:0 0 6px;color:#9ca3af;font-family:Georgia,serif;font-size:13px;line-height:1.5;">${escapeHtml(shipping.relayName)}${addr ? '<br>' + escapeHtml(addr) : ''}</td>
                  </tr>`;
      text.push(`  Point relais : ${shipping.relayName}${addr ? ', ' + addr : ''}`);
    }
  }
...
<body style="margin:0;padding:0;background-color:#050a14;background-image:url('https://oradia.fr/images/oradia-hero-4k.webp');background-size:cover;background-position:center top;" bgcolor="#050a14">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" background="https://oradia.fr/images/oradia-hero-4k.webp" bgcolor="#050a14" style="background-color:#050a14;background-image:url('https://oradia.fr/images/oradia-hero-4k.webp');background-size:cover;background-position:center top;">
    <tr>
...
// items / shipping (facultatifs) : récapitulatif détaillé d'une précommande. Sans eux
// (anciens appels, dons), seule l'offre principale est affichée.
async function sendBrevoEmail({ toEmail, toName, offer, amountTotal, invoiceUrl = null, items = null, shipping = null }) {
    try {
        ...
        const amountLabel = String(amountTotal).replace('.', ',');
        const recap = isDonation
            ? { html: '', text: `- ${describeOffer(offer)}` }
            : buildOrderRecap({ offer, items, shipping });
        const subject = isDonation
            ? "Rudy d'Oradia - Merci pour ton soutien à ORADIA"
            : "Rudy d'Oradia - Ta précommande ORADIA est confirmée";
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#0f1d35">
                  <tr>
                    <td align="center" style="padding:18px 24px;">
                      ...
                      <a href="${escapeHtml(invoiceUrl)}" class="btn" style="display:inline-block;background-color:#d4af37;color:#0a1628;font-family:Georgia,serif;font-size:14px;font-weight:bold;text-decoration:none;padding:12px 24px;letter-spacing:0.5px;">Télécharger la facture PDF</a>
                    </td>
                  </tr>
                </table>
              </td>
        ...
        const body = `
          <p style="margin:0 0 20px;color:#e8e9eb;font-family:Georgia,serif;font-size:15px;line-height:1.8;">${safeName ? safeName + ',' : 'Cher(e) ami(e),'}</p>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px;border-left:3px solid #d4af37;" bgcolor="#0f1d35">
            <tr>
              <td style="padding:22px 28px;">
                <p style="margin:0 0 10px;color:#9ca3af;font-family:Georgia,serif;font-size:12px;text-transform:uppercase;letter-spacing:1px;">${isDonation ? 'Contribution' : 'Ta commande'}</p>
                ${isDonation
                  ? `<p style="margin:0 0 16px;color:#f0c75e;font-family:Georgia,serif;font-size:20px;font-weight:bold;line-height:1.3;">${escapeHtml(describeOffer(offer))}</p>`
                  : `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 10px;">${recap.html}
                </table>`}
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:14px 0 0;">
                  <tr>
                    <td style="color:#9ca3af;font-family:Georgia,serif;font-size:13px;text-transform:uppercase;letter-spacing:1px;">Total payé</td>
                    <td align="right" style="color:#e8e9eb;font-family:Georgia,serif;font-size:17px;font-weight:bold;">${escapeHtml(amountLabel)} €</td>
                  </tr>
                </table>
              </td>
            </tr>
...
// Construit l'objet shipping de sendBrevoEmail à partir d'une ligne preorders.
function shippingFromOrder(order) {
  if (!order || !order.shipping_method) return null;
  return {
    method: order.shipping_method,
    priceCents: order.shipping_price_cents,
    relayName: order.relay_name,
    relayAddress: order.relay_address1,
    relayPostalCode: order.relay_postal_code,
    relayCity: order.relay_city
  };
}
...
async function sendShippingEmail({ toEmail, toName, trackingNumber }) {
    try {
        ...
        const body = `
          <p style="margin:0 0 20px;color:#e8e9eb;font-family:Georgia,serif;font-size:15px;line-height:1.8;">${toName ? escapeHtml(toName) + ',' : 'Cher(e) ami(e),'}</p>
          <p style="margin:0 0 24px;color:#d1d5db;font-family:Georgia,serif;font-size:14px;line-height:1.9;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px;border-left:3px solid #d4af37;" bgcolor="#0f1d35">
            <tr>
              <td style="padding:22px 28px;">
                ...
                <p style="margin:0;color:#f0c75e;font-family:Georgia,serif;font-size:20px;font-weight:bold;">${escapeHtml(trackingNumber)}</p>
              </td>
            </tr>
          </table>
        ...
        const response = await fetch('https://api.brevo.com/v3/smtp/email', {
            ...
            body: JSON.stringify({
                ...
                to: [{ email: toEmail, name: toName }],
                replyTo: { email: 'contact@oradia.fr', name: 'Oradia' },
                subject: "Rudy d'Oradia - Ta commande ORADIA est en chemin",
                htmlContent: buildEmailHtml({
                    title: 'En chemin',
    ...
    try {
        ...
        const response = await fetch('https://api.brevo.com/v3/smtp/email', {
            ...
            body: JSON.stringify({
                ...
                to: [{ email: toEmail, name: toName }],
                replyTo: { email: 'contact@oradia.fr', name: 'Oradia' },
                subject: "Rudy d'Oradia - Ton Oracle ORADIA est prêt à être récupéré",
                htmlContent: buildEmailHtml({
                    title: 'Prêt à partir',
...
async function sendRefundEmail({ toEmail, toName, offer, amountEur }) {
    try {
        if (!process.env.BREVO_API_KEY || !process.env.BREVO_SENDER_EMAIL) {
            console.error('Configuration Brevo manquante');
            return false;
        ...
        const body = `
          <p style="margin:0 0 20px;color:#e8e9eb;font-family:Georgia,serif;font-size:15px;line-height:1.8;">${toName ? escapeHtml(toName) + ',' : 'Cher(e) ami(e),'}</p>
          <p style="margin:0 0 24px;color:#d1d5db;font-family:Georgia,serif;font-size:14px;line-height:1.9;">
            L'objectif de précommandes n'a pas été atteint dans les délais prévus. Conformément à l'engagement pris lors de ta précommande, tu es intégralement remboursé(e) — aucune démarche n'est nécessaire de ta part.
          </p>

          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px;border-left:3px solid #d4af37;" bgcolor="#0f1d35">
            <tr>
              <td style="padding:22px 28px;">
                <p style="margin:0 0 10px;color:#9ca3af;font-family:Georgia,serif;font-size:12px;text-transform:uppercase;letter-spacing:1px;">Montant remboursé</p>
                <p style="margin:0 0 16px;color:#f0c75e;font-family:Georgia,serif;font-size:20px;font-weight:bold;line-height:1.3;">${escapeHtml(offer ? describeOffer(offer) : 'Précommande ORADIA')}</p>
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                  <tr><td height="1" bgcolor="#1a2d47" style="line-height:1px;font-size:1px;">&nbsp;</td></tr>
                </table>
                <p style="margin:14px 0 0;color:#e8e9eb;font-family:Georgia,serif;font-size:17px;font-weight:bold;">${amountEur} €</p>
              </td>
            </tr>
          </table>

          <p style="margin:0;color:#d1d5db;font-family:Georgia,serif;font-size:14px;line-height:1.9;">
            Le remboursement apparaîtra sur ton relevé bancaire sous quelques jours (délai habituel de ta banque). Merci sincèrement d'avoir soutenu ce projet — j'espère avoir l'occasion de te retrouver pour une prochaine étape d'ORADIA.
          </p>`;
        ...
        const response = await fetch('https://api.brevo.com/v3/smtp/email', {
            ...
            body: JSON.stringify({
                ...
                to: [{ email: toEmail, name: toName }],
                replyTo: { email: 'contact@oradia.fr', name: 'Oradia' },
                subject: "Rudy d'Oradia - Ta précommande ORADIA a été remboursée",
                htmlContent: buildEmailHtml({
                    title: 'Remboursement',
                    subtitle: "Ta précommande t'a été intégralement remboursée",
                    body
                }),
                textContent: `Ta précommande ORADIA a été remboursée\n\nBonjour${toName ? ' ' + toName : ''},\n\nL'objectif de précommandes n'a pas été atteint dans les délais prévus. Tu es remboursé(e) de ${amountEur}€, sans démarche de ta part.\n\nMerci pour ton soutien,\nRudy\noradia.fr`
            })
        });

        if (!response.ok) { console.error(`Brevo API error: ${response.status}`); return false; }
        console.log('Refund email sent via Brevo');
        return true;
    } catch (error) {
        console.error('Failed to send refund email via Brevo:', error.message);
        return false;
    }
}

module.exports = { sendBrevoEmail, sendShippingEmail, sendExportEmail, sendReadyEmail, sendRefundEmail, describeOffer, shippingFromOrder };

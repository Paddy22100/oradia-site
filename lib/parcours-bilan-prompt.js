...
## Intention suggérée
Reformule l'idée de la section précédente en UNE seule question directement utilisable comme intention pour un nouveau tirage. Règles strictes, sans exception :
- Une seule phrase, une seule question (un seul "?", jamais deux questions dans la même phrase)
- Commence obligatoirement par "Comment", "Pourquoi", "Quel", "Quelle", "Quels", "Quelles" ou "À quoi"
- Se termine par "?" et rien après
- Entre 8 et 20 mots, sans guillemets, sans note d'encouragement, sans contexte autour
- Exemple de format attendu : "Comment puis-je accueillir le changement qui s'annonce dans ma vie professionnelle ?"
...
// Reprend exactement les règles de validation du champ intention de tore.html
// (isOpenQuestion / hasMultipleQuestions) : si le modèle n'a pas respecté le
// format demandé pour "Intention suggérée", mieux vaut ne rien proposer que
// d'envoyer un texte qui échouera silencieusement la checklist sur tore.html.
function isValidSuggestedIntention(text) {
  const s = (text || '').trim();
  if (!s || !s.endsWith('?')) return false;
  if ((s.match(/\?/g) || []).length > 1) return false;
  const words = s.split(/\s+/).length;
  if (words < 4 || words > 30) return false;
  const lower = s.toLowerCase();
  const closedStarters = ['est-ce que', 'peux-tu', 'puis-je', 'as-tu', 'avez-vous', 'es-tu', 'êtes-vous', 'dois-je', 'peut-on', 'sera-t-il', 'serait-il'];
  if (closedStarters.some(p => lower.startsWith(p))) return false;
  const openStarters = ['comment', 'pourquoi', 'quel ', 'quelle ', 'quels ', 'quelles ', 'à quoi'];
  return openStarters.some(p => lower.startsWith(p));
}

// Découpe la réponse en { recurrent, evolution, question, suggestedIntention }
// à partir des titres "## ...".
function splitBilanSections(fullText) {
  const result = { recurrent: '', evolution: '', question: '', suggestedIntention: '' };
  ...
  matches.forEach((match, index) => {
    ...
    if (title.includes('intention')) result.suggestedIntention = content.replace(/^["«]|["»]$/g, '').trim();
  });

  if (!isValidSuggestedIntention(result.suggestedIntention)) result.suggestedIntention = '';
...
module.exports = { MIN_TIRAGES_FOR_BILAN, buildParcoursPrompt, generateParcoursBilan, splitBilanSections, isValidSuggestedIntention };

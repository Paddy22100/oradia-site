...
async function getStripeFeesDetail(startDate, endDate) {
    ...
    try {
        ...
        const params = {
            created: { gte: toUnixSeconds(startDate), lt: toUnixSeconds(endDate) },
            limit: 100,
            // Expanse la source sous-jacente pour retrouver le PaymentIntent — c'est la
            // seule référence Stripe que preorders/donors stockent (payment_intent_id).
            // Selon la version d'API/le type de transaction, cette source peut être soit
            // une Charge (auquel cas son champ payment_intent donne l'ID), soit déjà le
            // PaymentIntent lui-même (dashboard Stripe : "ID de paiement" = pi_... — vu
            // en pratique sur ce compte) : les deux cas sont gérés.
            expand: ['data.source']
        };

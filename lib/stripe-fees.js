...
/**
 * Frais Stripe réel de chaque encaissement sur une période, avec son PaymentIntent ET
 * son Invoice (si applicable) — même source (balance transactions) que
 * getStripeFeesForPeriod, mais ligne par ligne au lieu d'un seul total agrégé. Sans
 * dépendre d'un export CSV qui se périme dès la transaction suivante — l'API est
 * interrogée à la demande, toujours à jour.
 *
 * Deux références différentes selon le type de vente, toutes deux nécessaires pour
 * rattacher le frais à la bonne ligne de transactions :
 * - precommande/don : transactions.source_ref = session Stripe, retrouvée depuis le
 *   PaymentIntent via preorders/donors.payment_intent_id (voir matchAndApplyStripeFees).
 * - abonnement (renouvellement) : transactions.source_ref = l'Invoice Stripe directement
 *   (api/stripe-webhook.js), donc invoiceId permet un rattachement direct sans étape
 *   intermédiaire. Le tout premier paiement d'un abonnement (Checkout Session), lui,
 *   est enregistré sous l'ID de session — non couvert ici, aucune table ne fait le lien
 *   session↔PaymentIntent pour les abonnements.
 *
 * @returns {Promise<{ok: true, details: Array<{paymentIntentId: string, invoiceId: string|null,
 *                     feeEur: number, netEur: number, amountEur: number}>} | {ok: false, error: string}>}
 */
async function getStripeFeesDetail(startDate, endDate) {
    ...
    try {
        ...
        const params = {
            created: { gte: toUnixSeconds(startDate), lt: toUnixSeconds(endDate) },
            limit: 100,
            // Expanse la source sous-jacente pour retrouver PaymentIntent et Invoice.
            // Selon la version d'API/le type de transaction, cette source peut être soit
            // une Charge (les deux champs sont dessus), soit déjà le PaymentIntent
            // lui-même (dashboard Stripe : "ID de paiement" = pi_... — vu en pratique
            // sur ce compte, qui a aussi un champ invoice) : les deux cas sont gérés.
            expand: ['data.source']
        };
        ...
        for await (const bt of stripe.balanceTransactions.list(params)) {
            ...
            if (source && typeof source === 'object') {
                if (source.object === 'payment_intent') {
                    ...
                    paymentIntentId = typeof source.payment_intent === 'string' ? source.payment_intent : source.payment_intent.id;
                }
                if (source.invoice) {
                    invoiceId = typeof source.invoice === 'string' ? source.invoice : source.invoice.id;
                }
            } else if (typeof source === 'string' && source.startsWith('pi_')) {
                paymentIntentId = source;
            }
            if (!paymentIntentId && !invoiceId) continue;
            details.push({
                paymentIntentId,
                invoiceId,

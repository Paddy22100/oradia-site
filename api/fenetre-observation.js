// api/fenetre-observation.js
// Endpoint serverless Vercel
// POST : active une fenêtre d'observation (stockage Supabase + email Brevo immédiat)
...
export default async function handler(req, res) {
  ...
  if (error) {
    console.error('[fenetre-observation] Supabase error:', error);
...
<body style="margin:0;padding:0;background:#0a192f;font-family:'Georgia',serif;">
  <div style="max-width:560px;margin:0 auto;padding:40px 24px;">
    ...
    <!-- Titre -->
    <h1 style="font-family:'Georgia',serif;font-size:26px;color:#d4af37;text-align:center;margin:0 0 8px;font-weight:normal;">
      Votre fenêtre d'observation
    </h1>
    <p style="text-align:center;color:rgba(212,175,55,0.5);font-size:13px;margin:0 0 28px;">
      est ouverte pour ${durationDays} jour${durationDays > 1 ? 's' : ''}
    </p>
    ...
    <div style="background:rgba(5,20,40,0.8);border:1px solid rgba(212,175,55,0.25);border-radius:12px;padding:20px 24px;margin-bottom:24px;">
      <p style="color:#d4af37;font-size:11px;letter-spacing:0.15em;text-transform:uppercase;margin:0 0 14px;">
        Points d'attention pour ces ${durationDays} jour${durationDays > 1 ? 's' : ''}
      </p>
    ...
    <div style="text-align:center;margin-bottom:28px;">
      <p style="color:rgba(212,175,55,0.6);font-size:13px;margin:0;">
        Vous recevrez un message de clôture le
      </p>
      <p style="color:#d4af37;font-size:15px;font-weight:bold;margin:4px 0 0;">
        ${closesAtFR}
      </p>
    </div>
    ...
    <p style="color:rgba(212,175,55,0.35);font-size:11px;text-align:center;line-height:1.6;margin:0;">
      Ce message vous a été envoyé par Oracle Oradia suite à votre tirage.<br>
  ...
  try {
    ...
    console.error('[fenetre-observation] Brevo error:', emailErr);
    // Ne pas bloquer — la fenêtre est créée même si l'email échoue
  }
